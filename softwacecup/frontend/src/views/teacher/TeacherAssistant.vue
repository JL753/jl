...
const runAssistant = async (customText) => {
  ...
  openTutorSSE({ question: askText, context: contextStr || JSON.stringify({}), answerMode: activeTool.value }, {
    onDelta(chunk) {
      markdown += chunk
    ...
    onError() {
      loading.value = false
      // Fallback mock response for demo when backend unavailable
      const mockResponse = getMockResponse(activeTool.value, askText)
      markdown = mockResponse
      assistantMsg.html = DOMPurify.sanitize(marked.parse(mockResponse))
      
      if (activeTool.value === 'plan') {
        planResult.value = mockResponse
        planResultHtml.value = DOMPurify.sanitize(marked.parse(mockResponse))
      }
      if (activeTool.value === 'ppt') {
        showPptGallery.value = true
      }
...
const generatePpt = () => {
  runAssistant(`请根据以下信息创作PPT：教材名称=${pptForm.textbook}，教学目标=${pptForm.target}，专业学科=${pptForm.major}，单元主题=${pptForm.unitTheme}，年级=${pptForm.grade}，知识点=${pptForm.points.join(',')}`)
}
