...
import { apiMyClasses, apiCreateClass, apiClassMembers } from '../../api/index.js'
...
onMounted(async () => {
  try {
    classes.value = (await apiMyClasses()).data || []
    for (const c of classes.value) {
      try { classMembers[c.id] = (await apiClassMembers(c.id)).data || [] } catch(e) { classMembers[c.id] = [] }
    }
  } catch(e) { /* ignore */ }
})
...
async function removeMember(classId, studentId) {
  try {
    await axios.delete(`/api/classes/${classId}/members/${studentId}`)
