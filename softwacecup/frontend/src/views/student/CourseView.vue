      ...
      <template v-else-if="currentSubChapter">
        ...
        <div v-show="contentTab === 'exercise'" class="content-section glass-card">
          ...
          <div v-else class="exercise-empty">
            ...
            <button class="generate-btn" @click="generatePractice" :disabled="practiceLoading">AI 生成练习题</button>
          </div>
        </div>
...
async function loadCourse() {
  ...
  try {
    ...
    console.error('[CourseView] loadCourse failed:', e)
...
async function loadSubChapter(subChapterId) {
  ...
  chatHistory.value = [{
    ...
    console.error('[CourseView] loadSubChapter failed:', e)
...
async function generatePractice() {
  ...
  try {
    ...
    if (parsed) { practiceQuestions.value = parsed; practiceLoading.value = false; return }
  } catch (e) {
  ...
  if (practiceQuestions.value.length === 0) {
    ElMessage.warning('\u7ec3\u4e60\u9898\u751f\u6210\u5931\u8d25\uff0c\u8bf7\u7a0d\u540e\u91cd\u8bd5')
  }
}
