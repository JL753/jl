...
onMounted(async () => {
  try {
    ...
    stats.value = { lessons: d.weeklyLessons || 0, exercises: d.weeklyExercises || 0, accuracy: d.weeklyAccuracy || d.accuracy || 0 }
...
function goPathStep(step) {
  ...
  } else if (step.subChapterId) {
    router.push(`/student/courses/0?sc=${step.subChapterId}`)
  } else {
    router.push('/student/subjects')
  }
}
